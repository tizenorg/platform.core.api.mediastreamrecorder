	...
	*=================================================================================*/
	if (type == MODE_LIVE_BUFFER_I420_VIDEO || type == MODE_LIVE_BUFFER_NV12_VIDEO_SW) {
		if (type == MODE_LIVE_BUFFER_I420_VIDEO) {
			err = streamrecorder_set_video_source_format(hmstreamrecorder->recorder, STREAMRECORDER_INPUT_FORMAT_I420);
			if (err < 0) {
				LOGE("Init fail. (%x)", err);
				goto ERROR;
			}
		} else if (type == MODE_LIVE_BUFFER_NV12_VIDEO_SW) {
			err = streamrecorder_set_video_source_format(hmstreamrecorder->recorder, STREAMRECORDER_INPUT_FORMAT_NV12);
			if (err < 0) {
				LOGE("Init fail. (%x)", err);
				goto ERROR;
			}
...
static gboolean mode_change()
{
	...
	while (!check) {
		...
		case '1':
			...
			streamrecorder_set_videosource_buffer(hmstreamrecorder->recorder);
			check = TRUE;
			break;
		case '2':
