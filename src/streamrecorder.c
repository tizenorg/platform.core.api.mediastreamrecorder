...
int streamrecorder_set_state_changed_cb(streamrecorder_h recorder, streamrecorder_state_changed_cb callback, void *user_data)
{
	...
	handle->user_cb[_STREAMRECORDER_EVENT_TYPE_STATE_CHANGE] = callback;
	handle->user_data[_STREAMRECORDER_EVENT_TYPE_STATE_CHANGE] = user_data;
...
int streamrecorder_unset_state_changed_cb(streamrecorder_h recorder)
{
	...
	handle->user_cb[_STREAMRECORDER_EVENT_TYPE_STATE_CHANGE] = NULL;
	handle->user_data[_STREAMRECORDER_EVENT_TYPE_STATE_CHANGE] = NULL;
...
int streamrecorder_set_interrupted_cb(streamrecorder_h recorder, streamrecorder_interrupted_cb callback, void *user_data)
{
	streamrecorder_s *handle = (streamrecorder_s *)recorder;
	...
	handle->user_cb[_STREAMRECORDER_EVENT_TYPE_INTERRUPTED] = callback;
	handle->user_data[_STREAMRECORDER_EVENT_TYPE_INTERRUPTED] = user_data;
...
int streamrecorder_unset_interrupted_cb(streamrecorder_h recorder)
{
	...
	handle->user_cb[_STREAMRECORDER_EVENT_TYPE_INTERRUPTED] = NULL;
	handle->user_data[_STREAMRECORDER_EVENT_TYPE_INTERRUPTED] = NULL;
...
int streamrecorder_set_size_limit(streamrecorder_h recorder, int kbyte)
{
	...
	ret = _streamrecorder_set_size_limit(recorder, kbyte);
	return ret;
}
...
int streamrecorder_set_time_limit(streamrecorder_h recorder, int second)
{
	...
	ret = _streamrecorder_set_time_limit(recorder, second);
	return ret;
}
...
int streamrecorder_get_size_limit(streamrecorder_h recorder, int *kbyte)
{
	...
	ret = _streamrecorder_get_size_limit(recorder, kbyte);
	return ret;
}
...
int streamrecorder_get_time_limit(streamrecorder_h recorder, int *second)
{
	...
	ret = _streamrecorder_get_time_limit(recorder, second);
	return ret;
}
...
static int __mm_streamrecorder_msg_cb(int message, void *param, void *user_data)
{
	...
	streamrecorder_policy_e policy = STREAMRECORDER_POLICY_NONE;
	...
	case MM_MESSAGE_STREAMRECORDER_STATE_CHANGED:
		...
		if (previous_state != handle->state && handle->user_cb[_STREAMRECORDER_EVENT_TYPE_STATE_CHANGE])
			((streamrecorder_state_changed_cb)handle->user_cb[_STREAMRECORDER_EVENT_TYPE_STATE_CHANGE])(previous_state, handle->state, policy, handle->user_data[_STREAMRECORDER_EVENT_TYPE_STATE_CHANGE]);

		if ((m->state.current == MM_STREAMRECORDER_STATE_PAUSED || m->state.current == MM_STREAMRECORDER_STATE_CREATED)) {
			if (handle->user_cb[_STREAMRECORDER_EVENT_TYPE_INTERRUPTED])
				((streamrecorder_interrupted_cb)handle->user_cb[_STREAMRECORDER_EVENT_TYPE_INTERRUPTED])(policy, previous_state, handle->state, handle->user_data[_STREAMRECORDER_EVENT_TYPE_INTERRUPTED]);
			else
	...
	case MM_MESSAGE_STREAMRECORDER_TIME_LIMIT:
		if (MM_MESSAGE_STREAMRECORDER_MAX_SIZE == message) {
			type = STREAMRECORDER_RECORDING_LIMIT_SIZE;
		} else if (MM_MESSAGE_STREAMRECORDER_NO_FREE_SPACE == message) {
			type = STREAMRECORDER_RECORDING_LIMIT_FREE_SPACE;
		} else {
			type = STREAMRECORDER_RECORDING_LIMIT_TIME;
		}
	...
	{
		void *consume = (m->consumed_mediabuffer).consumed_buffer;
