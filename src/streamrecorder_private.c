...
int __convert_streamrecorder_error_code(const char *func, int code)
{
	...
	case MM_ERROR_STREAMRECORDER_NOT_INITIALIZED:
		...
		break;
	case MM_ERROR_STREAMRECORDER_DEVICE:
		ret = STREAMRECORDER_ERROR_DEVICE;
...
static int _streamrecorder_check_and_set_attribute(streamrecorder_h recorder, const char *attribute_name, int set_value)
{
	...
	if (!strcmp(attribute_name, MMSTR_AUDIO_ENCODER)) {
		if (set_value == STREAMRECORDER_AUDIO_CODEC_DISABLE) {
			ret = mm_streamrecorder_set_attributes(handle->mm_handle, NULL,
												MMSTR_AUDIO_DISABLE, true,
												NULL);
		} else {
			ret = mm_streamrecorder_set_attributes(handle->mm_handle, NULL,
												MMSTR_AUDIO_DISABLE, false,
												MMSTR_AUDIO_ENCODER, set_value,
												NULL);
		}
	} else {
		ret = mm_streamrecorder_set_attributes(handle->mm_handle, NULL,
...
{
	int format_table[6] = { MM_FILE_FORMAT_3GP, /* STREAMRECORDER_FILE_FORMAT_3GP */
...
int _streamrecorder_set_audio_encoder(streamrecorder_h recorder, streamrecorder_audio_codec_e codec)
{
	...
	if (codec != STREAMRECORDER_AUDIO_CODEC_DISABLE &&
		(codec < STREAMRECORDER_AUDIO_CODEC_AMR || codec > STREAMRECORDER_AUDIO_CODEC_PCM)) {
		LOGE("invalid parameter : codec %d", codec);
		return STREAMRECORDER_ERROR_INVALID_PARAMETER;
	}

	if (handle->type == _STREAMRECORDER_TYPE_AUDIO && codec == STREAMRECORDER_AUDIO_CODEC_DISABLE) {
		LOGE("AUDIO_CODEC_DISABLE is not supported in audio mode");
		return STREAMRECORDER_ERROR_NOT_SUPPORTED;
	}

	return _streamrecorder_check_and_set_attribute(recorder, MMSTR_AUDIO_ENCODER, codec == STREAMRECORDER_AUDIO_CODEC_DISABLE ? STREAMRECORDER_AUDIO_CODEC_DISABLE : audio_table[codec]);
}
...
int _streamrecorder_get_audio_encoder(streamrecorder_h recorder, streamrecorder_audio_codec_e *codec)
{
	...
	if (ret == MM_ERROR_NONE && audio_disable == 0) {
		...
		*codec = STREAMRECORDER_AUDIO_CODEC_DISABLE;
	}
...
int _streamrecorder_get_video_encoder(streamrecorder_h recorder, streamrecorder_video_codec_e *codec)
{
	...
	if (ret == MM_ERROR_NONE) {
		...
		case MM_VIDEO_CODEC_H264:
			*codec = STREAMRECORDER_VIDEO_CODEC_H264;
			break;
